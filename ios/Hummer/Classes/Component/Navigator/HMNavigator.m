...
HM_EXPORT_CLASS_METHOD(openPage, openPage:
    callback:)

HM_EXPORT_CLASS_METHOD(popPage, __popPage:)

HM_EXPORT_CLASS_METHOD(popToPage, __popToPage:)

HM_EXPORT_CLASS_METHOD(popToRootPage, __popToRootPage:)

HM_EXPORT_CLASS_METHOD(popBack, __popBackWithCount:pageInfo:)
...
+ (void)openPage:(HMBaseValue *)params callback:(HMClosureType)callback {
...
+ (void)__popPage:(HMBaseValue *)params {
    BOOL animated = YES;
    NSDictionary<NSString *, id> *parameterDictionary = params.toDictionary;
    NSNumber *numberValue = params.toNumber;
    if (parameterDictionary) {
        ...
        animated = numberValue.boolValue;
    }
...
+ (void)__popBackWithCount:(HMBaseValue *)count pageInfo:(HMBaseValue *)pageInfo {
    ...
    __block uint32_t countValue = count.toNumber.unsignedIntValue;
...
+ (void)pushPageWithInfo:(HMNavigatorPageInfo *)info {
    ...
    if (![url.scheme containsString:@"http"] && [UIApplication.sharedApplication canOpenURL:url]) {
        if (@available(iOS 10, *)) {
