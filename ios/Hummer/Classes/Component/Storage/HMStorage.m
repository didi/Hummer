...
HM_EXPORT_CLASS_METHOD(set, __set:object:)

HM_EXPORT_CLASS_METHOD(get, __getObjectForKey:)

HM_EXPORT_CLASS_METHOD(remove,__removeObjectForKey:)
...
+ (NSNumber *)__set:(HMBaseValue *)key object:(HMBaseValue *)object {
    ...
    if ([object.executor valueIsNullOrUndefined:key] || keyString.length == 0) {
        return @(NO);
    }
    id<NSCoding> data = [object.executor convertValueToPortableObject:object];
    if (data) {
