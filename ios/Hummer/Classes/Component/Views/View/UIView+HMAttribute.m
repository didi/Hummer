...
- (void)set__borderRadius:(NSArray<NSValue *> *)list {
    ...
    // TODO(唐佳诚): 支持百分比特性
    NSNumber *topLeft = list[0].ygValue.unit == YGUnitPoint ? @(list[0].ygValue.value) : nil;
    NSNumber *topRight = list[1].ygValue.unit == YGUnitPoint ? @(list[1].ygValue.value) : nil;
    NSNumber *bottomRight = list[2].ygValue.unit == YGUnitPoint ? @(list[2].ygValue.value) : nil;
    NSNumber *bottomLeft = list[3].ygValue.unit == YGUnitPoint ? @(list[3].ygValue.value) : nil;
...
- (void)set__borderTopLeftRadius:(YGValue)radius {
    [self hm_saveCornerRadiusWithTopLeft:radius.unit == YGUnitPoint ? @(radius.value) : nil topRight:nil bottomLeft:nil bottomRight:nil];
}

//HMBorderPropertyYGValueSetter(Radius, TopRight, YGValue, radius)
- (void)set__borderTopRightRadius:(YGValue)radius {
    [self hm_saveCornerRadiusWithTopLeft:nil topRight:radius.unit == YGUnitPoint ? @(radius.value) : nil bottomLeft:nil bottomRight:nil];
}

//HMBorderPropertyYGValueSetter(Radius, BottomLeft, YGValue, radius)
- (void)set__borderBottomLeftRadius:(YGValue)radius {
    [self hm_saveCornerRadiusWithTopLeft:nil topRight:nil bottomLeft:radius.unit == YGUnitPoint ? @(radius.value) : nil bottomRight:nil];
}

//HMBorderPropertyYGValueSetter(Radius, BottomRight, YGValue, radius)
- (void)set__borderBottomRightRadius:(YGValue)radius {
    [self hm_saveCornerRadiusWithTopLeft:nil topRight:nil bottomLeft:nil bottomRight:radius.unit == YGUnitPoint ? @(radius.value) : nil];
}
...
- (void)set__backgroundImage:(NSString *)imageString {
    [self hm_updateImageWithString:imageString completionBlock:^(UIImage * _Nullable image) {
        // 内存托管对象
