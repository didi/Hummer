...
@property (nonatomic, weak, nullable) HMBaseValue *customContentViewValue;
...
{
    HMBaseValue *viewValue = (HMBaseValue *)view.hm_value;
    if (viewValue) {
        if (self.customContentViewValue) {
            id customContentViewObject = [self.customContentViewValue toNativeObject];
            if ([customContentViewObject isKindOfClass:UIView.class]) {
                [((UIView *) customContentViewObject) removeFromSuperview];
            }
        }
        self.customContentViewValue = viewValue;
    }
