...
//    _executor = [[HMHermesExecutor alloc] init];
    _executor = [[HMJSCExecutor alloc] init];
    [[HMJSGlobal globalObject] weakReference:self];
    _executor.exceptionHandler = ^(HMExceptionModel *exception) {
    ...
    [_executor evaluateWithScript:jsString sourceUrl:@"http://localhost:8080/builtIn.js"];
    ...
    [self.executor evaluateWithScript:[NSString stringWithFormat:@"(function(){hummerLoadClass(%@)})()", classesStr] sourceUrl:@"http://localhost:8080/hummer.js"];
...
- (HMBaseValue *)evaluateScript:(NSString *)javaScriptString fileName:(NSString *)fileName {
    ...
    return [self.executor evaluateWithScript:javaScriptString sourceUrl:fileName];
}
