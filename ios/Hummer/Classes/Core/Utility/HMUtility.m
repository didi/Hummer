...
UIWindow *_Nullable hm_key_window(void)
{
    ...
    if (@available(iOS 13, *)) {
        [UIApplication.sharedApplication.connectedScenes enumerateObjectsUsingBlock:^(UIScene * _Nonnull obj, BOOL * _Nonnull stop) {
            // 支持权限弹框情况
            if ((obj.activationState == UISceneActivationStateForegroundActive || obj.activationState == UISceneActivationStateForegroundInactive) && [obj isKindOfClass:UIWindowScene.class]) {
...
HMResourceModel *hm_resolveResource(NSString *path) {
    ...
    // `/` 根路径也被认为是 component 之一，如果存在的话
    [pathComponent enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        // 1. 是 bundle/app/framework 之一
        // 1.1 mainBundle
        // 1.2 相对路径 bundle
        // 2. 附加到 filePath 中
        if ([obj.pathExtension.lowercaseString isEqualToString:@"bundle"] || [obj.pathExtension.lowercaseString isEqualToString:@"framework"] || [obj.pathExtension.lowercaseString isEqualToString:@"app"]) {
            ...
            if (resourceModel.resourceBundle && ![bundlePath hasPrefix:@"/"]) {
                // 必须是相对路径才能在已有 bundle 的情况下转化
                // 实际上如果路径为 / 则 url 为 nil，苹果 API 不做处理，这里实际上不需要专门判断 hasPrefix:@"/"
            ...
            } else if (!resourceModel.resourceBundle && [bundlePath hasPrefix:@"/"] && [obj.pathExtension.lowercaseString isEqualToString:@"app"]) {
                // 尝试 mainBundle
...
NSMethodSignature *hm_extractMethodSignatureFromBlock(id block) {
