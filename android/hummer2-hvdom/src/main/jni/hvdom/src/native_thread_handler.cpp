...
////
//
#include "falcon/native_thread_handler.h"

void NativeThreadHandler::start() {
...
void NativeThreadHandler::sendMessage(const Message &message) {
...
void NativeThreadHandler::handleMessage(const Message &message) {
...
void *NativeThreadHandler::threadInitFunction(void *arg) {
    NativeThreadHandler *pNativeThread = static_cast<NativeThreadHandler *>(arg);
    pNativeThread->run();
    return nullptr;
}
