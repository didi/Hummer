...
void Inspector::addCurrentScriptToLoadedScripts() {
  ...
  if (!loadedScripts_.count(info.fileId)) {
    ...
    // bugfix: 修复刷新页面（重新执行一遍业务代码）后，之前设置的断点失效的问题
    std::vector<debugger::BreakpointID> ids = debugger_.getBreakpoints();
    for (size_t i = 0; i < ids.size(); i++) {
      debugger::SourceLocation loc = debugger_.getBreakpointInfo(ids[i]).resolvedLocation;
      if (info.fileId > 0 && info.fileName == loc.fileName) {
        loc.fileId = info.fileId;
        debugger_.deleteBreakpoint(ids[i]);
        debugger_.setBreakpoint(loc);
      }
    }
