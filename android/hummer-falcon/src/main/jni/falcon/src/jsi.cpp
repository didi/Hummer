...
void JsiObjectEx::createReference() {
    ...
    if (status != NAPIExceptionOK) {
        warn("JsiObjectEx::createReference() error. status=%d", status);
    }
}
...
void JsiObjectEx::deleteReference() {
    ...
    if (status != NAPIExceptionOK) {
        warn("JsiObjectEx::deleteReference() error. status=%d", status);
    }
}
...
JsiValue *JsiObjectEx::toJsiValue() {
    NAPIHandleScope scope;
    JSUtils::openHandleScope(&env_, &scope);
    ...
    JsiValue *jsiValue = JSUtils::toValue(&env_, &thisValue);
...
void JsiObjectEx::toJsValue(NAPIValue *value) {
    NAPIHandleScope scope;
    JSUtils::openHandleScope(&env_, &scope);
...
//    info("JsiObject::functionWrapper() result methodName=%s,argc=%d",jsiFunction->methodName,argc);
    ...
    if (jsiValue != nullptr) {
        JSUtils::toJSValue(&env, jsiValue, &result);
        delete jsiValue;
    }
    return result;
}
