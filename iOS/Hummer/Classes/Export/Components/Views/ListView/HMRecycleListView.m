...
#import "HMEventDefines.h"
#import "HMRecycleListView+JSInterface.h"
...
// 实际偏移量范围 - contentInset.top ～ self.contentSize.height + self.contentInset.bottom
- (void)scrollToX:(HMBaseValue *)xValue Y:(HMBaseValue *)yValue {
    ...
    [self safe_setContentOffset:CGPointMake(offsetX, offsetY) animated:YES];
}
...
- (void)scrollByX:(HMBaseValue *)xValue Y:(HMBaseValue *)yValue {
    ...
    CGFloat targetOffsetX = offset.x + offsetX;
    CGFloat targetOffsetY = offset.y + offsetY;
    
    [self safe_setContentOffset:CGPointMake(targetOffsetX, targetOffsetY) animated:YES];
}

- (void)safe_setContentOffset:(CGPoint)contentOffset animated:(BOOL)animated {
    
    CGFloat targetOffsetX = contentOffset.x;
    CGFloat targetOffsetY = contentOffset.y;
    
    //不需要考虑 contentInset.top、 contentInset.left，因为是叠加
    CGPoint maxOffset = CGPointMake(self.contentInset.right + self.contentSize.width - self.frame.size.width,
                                    self.contentInset.bottom + self.contentSize.height - self.frame.size.height);
    CGPoint minOffset = CGPointMake(0 - self.contentInset.left, 0 - self.contentInset.top);
    
    targetOffsetX = MAX(minOffset.x, MIN(maxOffset.x, targetOffsetX));
    targetOffsetY = MAX(minOffset.y, MIN(maxOffset.y, targetOffsetY));
    [self setContentOffset:CGPointMake(targetOffsetX, targetOffsetY) animated:animated];
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (self.scrollEvent) {
        self.scrollEvent(HMScrollEventName, event);
    }
    [self hm_notifyWithEventName:HMScrollEventName params:event];
}
