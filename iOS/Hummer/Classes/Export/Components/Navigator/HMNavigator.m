...
+ (void)pushPageWithInfo:(HMNavigatorPageInfo *)info {
    ...
    if (!info.closeSelf) {return;}
    UINavigationController *nav = nil;
    if ([topViewController isKindOfClass:UIViewController.class]) {
        nav = topViewController.navigationController;
    }else if ([topViewController isKindOfClass:UINavigationController.class]){
        nav = (UINavigationController *)topViewController;
    }
    if (!nav) {return;}
    __weak typeof(nav) wNav = nav;    
    [nav hummer_transitionAnimationCompletion:^(id<UIViewControllerTransitionCoordinatorContext>  _Nullable context) {
        
        __strong typeof(wNav) sNav = wNav;
        NSMutableArray *tempArr = [sNav.childViewControllers mutableCopy];
        if (tempArr.count >= 2) {
            //无法做校验 倒数第二个vc == closeSelf VC，因此直接移除前一个
            [tempArr removeObjectAtIndex:tempArr.count-2];
            [sNav setViewControllers:tempArr animated:NO];
        }
    }];
}
