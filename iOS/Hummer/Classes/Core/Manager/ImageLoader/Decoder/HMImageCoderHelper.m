...
void HMImageCopyAssociatedObject(UIImage * _Nullable source, UIImage * _Nullable target) {
    if (!source || !target) {
        return;
    }
    // Image Metadata
//    target.hm_imageLoopCount = source.hm_imageLoopCount;
    target.hm_imageFormat = source.hm_imageFormat;
    // Force Decode
    target.hm_isDecoded = source.hm_isDecoded;
    // Extended Cache Data
}
@implementation HMImageCoderHelper

+ (UIImage *)animatedImageWithFrames:(NSArray<HMImageFrame *> *)frames {
    ...
    [frames enumerateObjectsUsingBlock:^(HMImageFrame * _Nonnull frame, NSUInteger idx, BOOL * _Nonnull stop) {
...
+ (NSArray<HMImageFrame *> *)framesFromAnimatedImage:(UIImage *)animatedImage {
    if (!animatedImage) {
        return nil;
    }
    ...
    [animatedImages enumerateObjectsUsingBlock:^(UIImage * _Nonnull image, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if ([image isEqual:previousImage]) {
            ...
            HMImageFrame *frame = [HMImageFrame frameWithImage:previousImage duration:avgDuration * repeatCount];
...
+ (CGImageRef)CGImageCreateDecoded:(CGImageRef)cgImage orientation:(CGImagePropertyOrientation)orientation {
    ...
    // Apply transform
    CGAffineTransform transform = HMCGContextTransformFromOrientation(orientation, CGSizeMake(newWidth, newHeight));
...
+ (UIImage *)decodedImageWithImage:(UIImage *)image {
    ...
    HMImageCopyAssociatedObject(image, decodedImage);
    decodedImage.hm_isDecoded = YES;
    return decodedImage;
}
...
+ (UIImage *)decodedAndScaledDownImageWithImage:(UIImage *)image limitBytes:(NSUInteger)bytes {
    ...
    @autoreleasepool {
        ...
        HMImageCopyAssociatedObject(image, destImage);
        destImage.hm_isDecoded = YES;
        return destImage;
    }
}
...
+ (BOOL)shouldDecodeImage:(nullable UIImage *)image {
    ...
    // Avoid extra decode
    if (image.hm_isDecoded) {
        return NO;
    }
    // do not decode animated images
    if (image.hm_isAnimated) {
        return NO;
    }
...
static inline CGAffineTransform HMCGContextTransformFromOrientation(CGImagePropertyOrientation orientation, CGSize size) {
