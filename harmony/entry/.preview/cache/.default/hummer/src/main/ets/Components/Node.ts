...
import { domOperationError, invalidArg, unrecognizedMethod } from "@bundle:com.example.hummer/entry@hummer/ets/Utils/Error";
import { forceToString, nextTick } from "@bundle:com.example.hummer/entry@hummer/ets/Utils/Utils";
...
export class HMNode extends HMBase implements IDomInstruction {
    ...
    private processFixed(superNode: HMNode): boolean {
    ...
    getRect(callback: fn) {
        let modePosition: componentUtils.ComponentInfo = componentUtils.getRectangleById(this.uniqueId);
        nextTick(() => {
