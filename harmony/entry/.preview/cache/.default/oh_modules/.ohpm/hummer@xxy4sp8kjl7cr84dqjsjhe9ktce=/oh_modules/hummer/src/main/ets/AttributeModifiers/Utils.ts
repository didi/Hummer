import { convertColorToARGB, isUndefined } from "@package:pkg_modules/.ohpm/hummer@xxy4sp8kjl7cr84dqjsjhe9ktce=/pkg_modules/hummer/src/main/ets/Utils/Utils";
import type { IHummerStyle } from '../Style/IHummerStyle';
import { hm2vp as tshm2vp, getVP as tsgetVP } from "@package:pkg_modules/.ohpm/hummer@xxy4sp8kjl7cr84dqjsjhe9ktce=/pkg_modules/hummer/src/main/ets/Utils/Utils";
...
export function getShadow(val?: string): ShadowOptions | undefined {
    ...
    const offsetX = getVP(styles[0]);
    const offsetY = getVP(styles[1]);
    const radius = getVP(styles[2]);
...
export function getLinearGradient(color?: string): LinearGradient | undefined {
    ...
    const linearGradient: LinearGradient = {
        angle: colors[0],
        colors: [[convertColorToARGB(colors[1]), 0], [convertColorToARGB(colors[2]), 1]] // 数组末尾元素占比小于1时满足重复着色效果
    };
    return linearGradient;
}
export function hm2vp(length: string, factor: number = 750): number {
    return tshm2vp(length, factor);
}
export function getVP(length: string | number): number | undefined {
    return tsgetVP(length);
}
